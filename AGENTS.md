...
华为昇腾 NPU 910B 上的 PaddleOCR-VL + PP-DocLayoutV2 文档解析环境。核心能力封装在 `docparse` Python 库（三条可切换解析管线），容器内通过 `python -m docparse` CLI 使用，宿主机轻量命令可用 `uv run docparse`；vLLM Ascend 后端提供 PaddleOCR-VL 推理服务。
...
├── docker-compose.yml      # docparse-npu + vllm-ascend 双服务编排（含 healthcheck）
├── config.yaml             # ★ 主配置：模型名/路径、引擎构造参数（版本升级入口）
├── pyproject.toml          # 宿主机 uv run 支持（轻量依赖 + docparse 命令入口）
├── requirements.txt        # 容器内 Python 依赖（paddleocr/openai 等）
├── docparse/               # ★ 核心库
...
| Task | Location | Notes |
|------|----------|-------|
| Docker 构建 | `Dockerfile`, `docker-compose.yml` | 镜像 `docparse-npu:v1`，容器 `docparse-dev` |
| vLLM 启动 | `scripts/start_vllm.sh` | VLLM_MODEL_PATH/VLLM_PORT/VLLM_MODEL_NAME 等可覆盖 |
| 配置/模型版本调整 | `config.yaml` + `docparse/config.py` | 优先级：环境变量 > config.yaml > 默认 |
| 管线构造 | `docparse/pipelines.py` | build_pipeline_kwargs / create_pipeline；引擎段未知参数透传 |
| 批量推理 | `docparse/batch.py` + CLI `parse` | PDF → JSON + Markdown + batch_report.json |
| OCR API 调用 | `docparse/client.py` | VLMOcrClient / check_vllm / wait_vllm |
| 宿主机轻量运行 | `pyproject.toml` | `uv run docparse ...`（仅 ocr/check/benchmark，无 NPU） |
...
```bash
# 构建镜像（compose 含 build 配置，自动打 tag docparse-npu:v1）
docker compose build
# 或构建+启动一步完成
docker compose up -d --build

# 启动全部服务（vLLM 健康后自动启动 docparse-npu）
docker compose up -d

# 只启动开发容器（不等 vLLM）
docker compose up -d --no-deps docparse-npu
...
python -m docparse parse                          # 批量解析 INPUT_PDF_DIR
python -m docparse parse input/pdfs/xxx.pdf --engine structure
...
- 完整验收流程见 README.md「完整验收流程」章节
- 旧 `scripts/batch_inference_pdfs.py` 已由 `python -m docparse parse` 取代；包名 `ppdoc` 于 2026-09-24 更名为 `docparse`
